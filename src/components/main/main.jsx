...
const MainPage = ({ profile, setProfile }) => {
  ...
  const getProfile = async (e) => {
    ...
    try {
      const response = await fetch("http://13.125.65.66:8000/api/profile/", {
        method: "GET",
        headers: {
  ...
  const onLoadMeals = async (e) => {
    try {
      const response = await fetch(
        `http://13.125.65.66:8000/api/food-intake/?date=${currentDate}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
...
const MainPageContent = ({
  ...
  const baseURL = "http://13.125.65.66:8000/api/food-intake/";
